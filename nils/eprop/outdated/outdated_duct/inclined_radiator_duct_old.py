...
# =============================================================================
# NILS: from docs\tutorial\section.py
...
if __name__== "__main__":
    ...
    inward = BasicSurface(n_sec=2, name='out', nn=201, ns=51, projection = False)
    
    ### NILS: intake
    
    x_centroid_intake = x_centroid_hx - l_hx/2 - l_up_duct
    y_centroid_intake = y_centroid_hx
    z_centroid_intake = dz_hx_intake
    ...
    # inward.secs[0].curve[0].curve_x = upper_sec.x
    # inward.secs[0].curve[0].curve_y = upper_sec.y
    # inward.secs[0].curve[0].curve_z = upper_sec.z
    # inward.secs[0].curve[1].curve_x = lower_sec.x
    # inward.secs[0].curve[1].curve_y = lower_sec.y
    # inward.secs[0].curve[1].curve_z = lower_sec.z
    inward.secs[0].curve[0].curve_x = x_centroid_intake * np.ones_like(x_)
    inward.secs[0].curve[0].curve_y = -w_intake/2 + x_
    inward.secs[0].curve[0].curve_z = z_centroid_intake + y_
    inward.secs[0].curve[1].curve_x = x_centroid_intake * np.ones_like(x_)
    inward.secs[0].curve[1].curve_y = -w_intake/2 + x_
    inward.secs[0].curve[1].curve_z = z_centroid_intake - y_
    inward.secs[0].join_curve()
    
    # # =============================================================================
    # inward.secs[0] = Section(n_curve = 4)
    # inward.secs[0].curve[0] = curve(
    #     r0=np.array([x_centroid_intake, y_centroid_intake + w_intake/2, z_centroid_intake - h_intake/2]),
    #     r1=np.array([x_centroid_intake, y_centroid_intake + w_intake/2, z_centroid_intake + h_intake/2]),
    #     nn=125,
    # )
    # inward.secs[0].curve[1] = curve(
    #     r0=np.array([x_centroid_intake, y_centroid_intake + w_intake/2, z_centroid_intake + h_intake/2]),
    #     r1=np.array([x_centroid_intake, y_centroid_intake - w_intake/2, z_centroid_intake + h_intake/2]),
    #     nn=125,
    # )
    # inward.secs[0].curve[2] = curve(
    #     r0=np.array([x_centroid_intake, y_centroid_intake - w_intake/2, z_centroid_intake + h_intake/2]),
    #     r1=np.array([x_centroid_intake, y_centroid_intake - w_intake/2, z_centroid_intake - h_intake/2]),
    #     nn=125,
    # )
    # inward.secs[0].curve[3] = curve(
    #     r0=np.array([x_centroid_intake, y_centroid_intake - w_intake/2, z_centroid_intake - h_intake/2]),
    #     r1=np.array([x_centroid_intake, y_centroid_intake + w_intake/2, z_centroid_intake - h_intake/2]),
    #     nn=125,
    # )
    ...
    ### NILS: upstream duct outlet/HX inlet
    inward.secs[1] = Section(n_curve = 4)
    inward.secs[1].curve[0] = curve(
        r0=np.array([x_centroid_hx + dx_hx_corner, y_centroid_hx + w_hx/2, z_centroid_hx - h_hx/2]),
        r1=np.array([x_centroid_hx - l_hx/2, y_centroid_hx + w_hx/2, z_centroid_hx + dz_hx_corner]),
        nn=125,
    )
    inward.secs[1].curve[1] = curve(
        r0=np.array([x_centroid_hx - l_hx/2, y_centroid_hx + w_hx/2, z_centroid_hx + dz_hx_corner]),
        r1=np.array([x_centroid_hx - l_hx/2, y_centroid_hx - w_hx/2, z_centroid_hx + dz_hx_corner]),
        nn=125,
    )
    inward.secs[1].curve[2] = curve(
        r0=np.array([x_centroid_hx - l_hx/2, y_centroid_hx - w_hx/2, z_centroid_hx + dz_hx_corner]),
        r1=np.array([x_centroid_hx + dx_hx_corner, y_centroid_hx - w_hx/2, z_centroid_hx - h_hx/2]),
        nn=125,
    )
    ...
    # # =============================================================================
    # inward.secs[1] = Section(n_curve = 2)
    # inward.secs[1].curve[0].curve_x = x_centroid_intake * np.ones_like(x_) + 1
    # inward.secs[1].curve[0].curve_y = -w_intake/2 + x_
    # inward.secs[1].curve[0].curve_z = z_centroid_intake + y_ + 0.1
    # inward.secs[1].curve[1].curve_x = x_centroid_intake * np.ones_like(x_) + 1
    # inward.secs[1].curve[1].curve_y = -w_intake/2 + x_
    # inward.secs[1].curve[1].curve_z = z_centroid_intake - y_ + 0.1
    # inward.secs[1].join_curve()
    # # =============================================================================
    ...
    # # NILS: upstream duct inlet
    # inward.secs[2] = Section(n_curve = 4)
    # inward.secs[2].curve[0] = curve(r0=np.array([-1.340,3.025,20.307]), r1=np.array([-1.340,1.505,20.307]), nn=125)
    # inward.secs[2].curve[1] = curve(r0=np.array([-1.340,1.505,20.307]), r1=np.array([-1.591,1.505,20.307]), nn=125)
    # inward.secs[2].curve[2] = curve(r0=np.array([-1.591,1.505,20.307]), r1=np.array([-1.591,3.025,20.307]), nn=125)
    # inward.secs[2].curve[3] = curve(r0=np.array([-1.591,3.025,20.307]), r1=np.array([-1.340,3.025,20.307]), nn=125)
    # inward.secs[2].join_curve()

    # NILS: fan inlet
    inward.secs[3] = Section(n_curve = 4)
    inward.secs[3].circle(np.array([-1.025,2.507]),np.array([-1.025,2.023]),np.array([-1.305,2.023]), nn=500)
    inward.secs[3].z = 23.000 * np.ones_like(inward.secs[3].x)
    #将曲线排序
    inward.secs[3].x,inward.secs[3].y,inward.secs[3].z = order_curve3d(inward.secs[3].x,inward.secs[3].y,inward.secs[3].z)
    #让曲线封闭
    ...
    inward.secs[4].z = 24.907 * np.ones_like(inward.secs[4].x)
    #将曲线排序
    ...
    inward.secs[5].curve[0] = curve(r0=np.array([-1.045,2.765,25.670]), r1=np.array([-1.045,1.765,25.670]), nn=125)
    inward.secs[5].curve[1] = curve(r0=np.array([-1.045,1.765,25.670]), r1=np.array([-1.323,1.765,25.670]), nn=125)
    inward.secs[5].curve[2] = curve(r0=np.array([-1.323,1.765,25.670]), r1=np.array([-1.323,2.765,25.670]), nn=125)
    inward.secs[5].curve[3] = curve(r0=np.array([-1.323,2.765,25.670]), r1=np.array([-1.045,2.765,25.670]), nn=125)
    inward.secs[5].join_curve()

    # NILS: HX inlet
    #inlet_x,inlet_y,inlet_z = read_sec_file('inward-outlet1.xlsx',0.001)
    inward.secs[6] = Section(n_curve = 4)
    inward.secs[6].curve[0] = curve(r0=np.array([-0.361,2.765,27.984]), r1=np.array([-0.361,1.765,27.984]), nn=125)
    inward.secs[6].curve[1] = curve(r0=np.array([-0.361,1.765,27.984]), r1=np.array([-1.372,1.765,26.234]), nn=125)
    inward.secs[6].curve[2] = curve(r0=np.array([-1.372,1.765,26.234]), r1=np.array([-1.372,2.765,26.234]), nn=125)
    inward.secs[6].curve[3] = curve(r0=np.array([-1.372,2.765,26.234]), r1=np.array([-0.361,2.765,27.984]), nn=125)
    inward.secs[6].join_curve()

    # NILS: HX outlet
    #sec_x, sec_y, sec_z = read_sec_file('inward-outlet2.xlsx',0.001)
    inward.secs[7] = Section(n_curve = 4)
    inward.secs[7].curve[0] = curve(r0=np.array([0,     -1.765+4.53,29.570]), r1=np.array([0,     -2.765+4.53,29.570]), nn=125)
    inward.secs[7].curve[1] = curve(r0=np.array([0,     -2.765+4.53,29.570]), r1=np.array([-1.445,-2.765+4.53,27.068]), nn=125)
    inward.secs[7].curve[2] = curve(r0=np.array([-1.445,-2.765+4.53,27.068]), r1=np.array([-1.445,-1.765+4.53,27.068]), nn=125)
    inward.secs[7].curve[3] = curve(r0=np.array([-1.445,-1.765+4.53,27.068]), r1=np.array([0,     -1.765+4.53,29.570]), nn=125)
    ...
    inward.smooth(0,1,smooth0=True,ratio_end=-1)
    # =============================================================================
    inward.smooth(1,3,smooth0=True,ratio_end=-1)
    inward.smooth(4,5,smooth0=True,ratio_end=-1)
    inward.smooth(5,7,smooth1=True,ratio_end=-1)

    inward.flip(axis = '+Y')
    inward.flip(axis = '+X')

    # inward.output_tecplot(fname = './dump/engine_inward.dat')
    inward.output_tecplot(fname='engine_inward_extended.dat')  # NILS
