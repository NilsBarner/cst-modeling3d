...
if __name__== "__main__":
    ...
    inward = BasicSurface(n_sec=8, name='out', nn=201, ns=51, projection = False)
    ...
    # =============================================================================
    # NILS: from https://cst-modeling3d.readthedocs.io/en/latest/source/tutorial/section.html
    
    sec = BasicSection(thick=None, chord=1.0, twist=0.0, lTwistAroundLE=False)
    ...
    # inward.secs[0].curve[0].curve_x = sec.z + 2  # sec.x
    # inward.secs[0].curve[0].curve_y = sec.y + 2.25
    # inward.secs[0].curve[0].curve_z = sec.x + 19  # sec.z
    # inward.secs[0].curve[1].curve_x = sec.z + 2  # sec.x
    # inward.secs[0].curve[1].curve_y = -sec.y + 2.25
    # inward.secs[0].curve[1].curve_z = sec.x + 19  # sec.z
    # inward.secs[0].join_curve()
    
    inward.secs[0] = Section(n_curve = 2)
    inward.secs[0].curve[0].curve_x = sec.y - 1.25
    inward.secs[0].curve[0].curve_y = sec.x + 0.5
    inward.secs[0].curve[0].curve_z = sec.z + 18
    inward.secs[0].curve[1].curve_x = -sec.y - 1.25
    inward.secs[0].curve[1].curve_y = sec.x + 0.5
    inward.secs[0].curve[1].curve_z = sec.z + 18
    inward.secs[0].join_curve()
    ...
    inward.secs[1] = Section(n_curve = 4)
    inward.secs[1].curve[0] = curve(r0=np.array([-1.174,3.025,19.368]), r1=np.array([-1.174,1.505,19.368]), nn=125)
    inward.secs[1].curve[1] = curve(r0=np.array([-1.174,1.505,19.368]), r1=np.array([-1.580,1.505,19.904]), nn=125)
    inward.secs[1].curve[2] = curve(r0=np.array([-1.580,1.505,19.904]), r1=np.array([-1.580,3.025,19.904]), nn=125)
    inward.secs[1].curve[3] = curve(r0=np.array([-1.580,3.025,19.904]), r1=np.array([-1.174,3.025,19.368]), nn=125)
    inward.secs[1].join_curve()
