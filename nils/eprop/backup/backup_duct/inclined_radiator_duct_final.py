...
if __name__== "__main__":
    ...
    # inward = BasicSurface(n_sec=10, name='out', nn=201, ns=51, projection = False)
    # inward = BasicSurface(n_sec=22, name='out', nn=201, ns=51, projection = False)
    inward = BasicSurface(n_sec=25, name='out', nn=201, ns=51, projection = False)
    ...
    for i, _slice in enumerate(slices_list):
        ...
        inward.secs[i] = Section(n_curve = 1)
        inward.secs[i].curve[0].curve_x = _slice['Z']
        inward.secs[i].curve[0].curve_y = _slice['Y']
        inward.secs[i].curve[0].curve_z = _slice['X']
        inward.secs[i].join_curve()
    
    # inward.geo(update_sec = False)
    
    # inward.smooth(0,9,smooth0=True,ratio_end=-1)

    # inward.flip(axis = '+Y')
    # inward.flip(axis = '+X')

    # inward.output_tecplot(fname='engine_inward_temp.dat')  # NILS
    ...
    inward.secs[1 + i_up_duct_out] = Section(n_curve = 4)
    inward.secs[1 + i_up_duct_out].curve[0] = curve(
    ...
    inward.secs[1 + i_up_duct_out].curve[1] = curve(
        r0=np.array([z_centroid_hx + dz_hx_corner, y_centroid_hx + w_hx/2, x_centroid_hx - l_hx/2]),
        r1=np.array([z_centroid_hx + dz_hx_corner, y_centroid_hx - w_hx/2, x_centroid_hx - l_hx/2]),
        # nn=125,
        nn=247,
    )
    inward.secs[1 + i_up_duct_out].curve[2] = curve(
        r0=np.array([z_centroid_hx + dz_hx_corner, y_centroid_hx - w_hx/2, x_centroid_hx - l_hx/2]),
        r1=np.array([z_centroid_hx - h_hx/2, y_centroid_hx - w_hx/2, x_centroid_hx + dx_hx_corner]),
        # nn=125,
        nn=3,
    )
    inward.secs[1 + i_up_duct_out].curve[3] = curve(
        r0=np.array([z_centroid_hx - h_hx/2, y_centroid_hx - w_hx/2, x_centroid_hx + dx_hx_corner]),
        r1=np.array([z_centroid_hx - h_hx/2, y_centroid_hx + w_hx/2, x_centroid_hx + dx_hx_corner]),
        # nn=125,
        nn=247,
    )
    inward.secs[1 + i_up_duct_out].join_curve()
    
    ### NILS: HX outlet
    inward.secs[2 + i_up_duct_out] = Section(n_curve = 4)
    inward.secs[2 + i_up_duct_out].curve[0] = curve(
        r0=np.array([z_centroid_hx - dz_hx_corner, y_centroid_hx + w_hx/2, x_centroid_hx + l_hx/2]),
        r1=np.array([z_centroid_hx + h_hx/2, y_centroid_hx + w_hx/2, x_centroid_hx - dx_hx_corner]),
    ...
    inward.secs[2 + i_up_duct_out].curve[1] = curve(
        r0=np.array([z_centroid_hx + h_hx/2, y_centroid_hx + w_hx/2, x_centroid_hx - dx_hx_corner]),
        r1=np.array([z_centroid_hx + h_hx/2, y_centroid_hx - w_hx/2, x_centroid_hx - dx_hx_corner]),
    ...
    inward.secs[2 + i_up_duct_out].curve[2] = curve(
        r0=np.array([z_centroid_hx + h_hx/2, y_centroid_hx - w_hx/2, x_centroid_hx - dx_hx_corner]),
        r1=np.array([z_centroid_hx - dz_hx_corner, y_centroid_hx - w_hx/2, x_centroid_hx + l_hx/2]),
        # r0=np.array([x_centroid_hx - dx_hx_corner, y_centroid_hx - w_hx/2, z_centroid_hx + h_hx/2]),
    ...
    inward.secs[2 + i_up_duct_out].curve[3] = curve(
        r0=np.array([z_centroid_hx - dz_hx_corner, y_centroid_hx - w_hx/2, x_centroid_hx + l_hx/2]),
        r1=np.array([z_centroid_hx - dz_hx_corner, y_centroid_hx + w_hx/2, x_centroid_hx + l_hx/2]),
        # r0=np.array([x_centroid_hx + l_hx/2, y_centroid_hx - w_hx/2, z_centroid_hx - dz_hx_corner]),
        # r1=np.array([x_centroid_hx + l_hx/2, y_centroid_hx + w_hx/2, z_centroid_hx - dz_hx_corner]),
    ...
    inward.secs[2 + i_up_duct_out].join_curve()
    ...
    for i, _slice in enumerate(slices_list):
        ...
        # Convert into `Section` in context of this script
        inward.secs[i + i_up_duct_out + 3] = Section(n_curve = 1)
        inward.secs[i + i_up_duct_out + 3].curve[0].curve_x = _slice['Z']
        inward.secs[i + i_up_duct_out + 3].curve[0].curve_y = _slice['Y']
        inward.secs[i + i_up_duct_out + 3].curve[0].curve_z = _slice['X']
        inward.secs[i + i_up_duct_out + 3].join_curve()
        
    # inward.geo(update_sec = False)
    
    # inward.smooth(0,9,smooth0=True,ratio_end=-1)
    # inward.smooth(12, 21,smooth0=True,ratio_end=-1)

    # inward.flip(axis = '+Y')
    # inward.flip(axis = '+X')

    # inward.output_tecplot(fname='engine_inward_temp_3.dat')  # NILS
    
    # # Plot sections
    
    # fig = plt.figure()
    # ax = fig.add_subplot(projection='3d')
    # for i in range(len(inward.secs)):
    #     ax.scatter(inward.secs[i].x, inward.secs[i].y, inward.secs[i].z)
    # ax.set_xlabel('x')
    # ax.set_ylabel('y')
    # ax.set_zlabel('z')
    # ax.set_aspect('equal')
    ...
    inward.secs[i_down_duct_out + 1] = Section(n_curve = 4)
    inward.secs[i_down_duct_out + 1].circle(
        np.array([y_centroid_fan_inlet - d_fan/2, z_centroid_fan_inlet]),
        np.array([y_centroid_fan_inlet + d_fan/2, z_centroid_fan_inlet]),
        np.array([y_centroid_fan_inlet, z_centroid_fan_inlet + d_fan/2]),
        nn=500,
    )
    inward.secs[i_down_duct_out + 1].z = x_centroid_fan_inlet * np.ones_like(inward.secs[i_down_duct_out + 1].x)
    #将曲线排序
    inward.secs[i_down_duct_out + 1].x, inward.secs[i_down_duct_out + 1].y, inward.secs[i_down_duct_out + 1].z = order_curve3d(
        inward.secs[i_down_duct_out + 1].x,
        inward.secs[i_down_duct_out + 1].y,
        inward.secs[i_down_duct_out + 1].z,
    )
    #让曲线封闭
    inward.secs[i_down_duct_out + 1].x[0] = inward.secs[i_down_duct_out + 1].x[-1]
    inward.secs[i_down_duct_out + 1].y[0] = inward.secs[i_down_duct_out + 1].y[-1]
    inward.secs[i_down_duct_out + 1].z[0] = inward.secs[i_down_duct_out + 1].z[-1]
    ...
    inward.secs[i_down_duct_out + 2] = Section(n_curve = 4)
    inward.secs[i_down_duct_out + 2].circle(
        np.array([y_centroid_fan_outlet - d_fan/2, z_centroid_fan_outlet]),
        np.array([y_centroid_fan_outlet + d_fan/2, z_centroid_fan_outlet]),
        np.array([y_centroid_fan_outlet, z_centroid_fan_outlet + d_fan/2]),
        nn=500,
    )
    inward.secs[i_down_duct_out + 2].z = x_centroid_fan_outlet * np.ones_like(inward.secs[i_down_duct_out + 2].x)
    #将曲线排序
    inward.secs[i_down_duct_out + 2].x, inward.secs[i_down_duct_out + 2].y, inward.secs[i_down_duct_out + 2].z = order_curve3d(
        inward.secs[i_down_duct_out + 2].x,
        inward.secs[i_down_duct_out + 2].y,
        inward.secs[i_down_duct_out + 2].z,
    )
    #让曲线封闭
    inward.secs[i_down_duct_out + 2].x[0] = inward.secs[i_down_duct_out + 2].x[-1]
    inward.secs[i_down_duct_out + 2].y[0] = inward.secs[i_down_duct_out + 2].y[-1]
    inward.secs[i_down_duct_out + 2].z[0] = inward.secs[i_down_duct_out + 2].z[-1]
    ...
    inward.secs[i_down_duct_out + 3] = Section(n_curve = 4)
    inward.secs[i_down_duct_out + 3].circle(
    ...
    inward.secs[i_down_duct_out + 3].z = x_centroid_nozzle_outlet * np.ones_like(inward.secs[i_down_duct_out + 3].x)
    #将曲线排序
    inward.secs[i_down_duct_out + 3].x, inward.secs[i_down_duct_out + 3].y, inward.secs[i_down_duct_out + 3].z = order_curve3d(
        inward.secs[i_down_duct_out + 3].x,
        inward.secs[i_down_duct_out + 3].y,
        inward.secs[i_down_duct_out + 3].z,
    )
    #让曲线封闭
    inward.secs[i_down_duct_out + 3].x[0] = inward.secs[i_down_duct_out + 3].x[-1]
    inward.secs[i_down_duct_out + 3].y[0] = inward.secs[i_down_duct_out + 3].y[-1]
    inward.secs[i_down_duct_out + 3].z[0] = inward.secs[i_down_duct_out + 3].z[-1]
    ...
    inward.smooth(0,9,smooth0=True,ratio_end=-1)
    # inward.smooth(12,21,smooth0=True,ratio_end=-1)
    inward.smooth(12,22,smooth0=True,ratio_end=-1)
    ...
    mesh = pv.read('engine_inward_new.dat')
