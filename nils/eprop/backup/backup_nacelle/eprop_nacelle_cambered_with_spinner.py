...
def generate_nacelle_from_radii_and_boattail_with_camber(
    ...
    # ---------------------------
    # --- SPINNER (MINIMAL ADD) ---
    # Spinner END located where vertical nacelle half-height b(psi) = LE_h.
    # Spinner then protrudes forward by spinner_length_frac * length.
    # ---------------------------
    
    spinner = None
    if LE_hor is not None and LE_hor > 0.0:
    
        # Find axial station where vertical half-height equals LE radius
        b_array = np.asarray(b)
        psi_array = np.asarray(psi)
    
        if LE_hor > b_array.max():
            idx_end = int(np.argmax(b_array))
            psi_attach = psi_array[idx_end]
        else:
            psi_attach = float(np.interp(LE_hor, b_array, psi_array))
        ...
        # If spinner length extremely small, create epsilon for plotting
        if c_sp <= 1e-6:
            c_sp = max(length * 0.01, 1e-3)
        ...
        if r_raw.max() > 0:
            r_scale = LE_hor / float(r_raw.max())
        else:
            r_scale = 1.0
        ...
        R_mesh = np.outer(r_vals, np.cos(theta))
        Z_mesh = np.outer(r_vals, np.sin(theta))
    
        # IMPORTANT: spinner now runs from x_tip → x_attach
        X_mesh = np.outer(x_tip + psih * c_sp, np.ones(n_theta))
    ...
    if plot:
        ...
        if spinner is not None:
            ...
            ax.plot_surface(spinner["x_mesh"], spinner["y_mesh"], spinner["z_mesh"],
...
if __name__ == "__main__":
    ...
    # if res["spinner"] is not None:
    #     print("Spinner chord (m):", res["spinner"]["c_sp"])
    #     print("Spinner psi_end:", res["spinner"]["psi_sp"])