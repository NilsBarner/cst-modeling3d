...
if __name__ == '__main__':
    ...
    if True:
        ...
        for tail, rLE, width_bump in [(0.0, 0.015, 1.0), (0.004, 0.005, 0.8)]:
            ...
            cst_u, cst_l, tmax, rLE_new = modify.set_leading_edge_radius(rLE_new=rLE, width_bump=width_bump)
    ...
    if True:
        ...
        for tail, slope_angle, width_bump in [(0.0, 15.0, 0.8), (0.004, 1.0, 0.2)]:
            ...
            # =============================================================================
            # Add TE flap based on section_flap() in cst_modeling/tools/auxiliary.py
            
            nn = len(geo_old.x)
            angle = -30
            ratio = 0.8
            xu_, xl_, yu_, yl_ = transform(geo_old.x, geo_old.x, geo_old.yu, geo_old.yl, rot=angle, x0=ratio, y0=0)
        
            iu1, iu2, _ = intersect_index(geo_old.x, geo_old.yu, xu_, yu_)
            il1, il2, _ = intersect_index(geo_old.x, geo_old.yl, xl_, yl_)
            nu_flap = nn - iu1
            nl_flap = nn - il1
        
            #* Adjust number of points on the flap
            xu_new2 = np.concatenate((geo_old.x[:iu1], xu_[iu2:]), axis=0)
            yu_new2 = np.concatenate((geo_old.yu[:iu1], yu_[iu2:]), axis=0)
            xl_new2 = np.concatenate((geo_old.x[:il1], xl_[il2:]), axis=0)
            yl_new2 = np.concatenate((geo_old.yl[:il1], yl_[il2:]), axis=0)
        
            xx_u = np.linspace(geo_old.x[iu1], xu_[-1], nu_flap)
            yy_u = interp_from_curve(xx_u, xu_new2, yu_new2)
            xu_new = np.concatenate((geo_old.x[:iu1], xx_u), axis=0)
            yu_new = np.concatenate((geo_old.yu[:iu1], yy_u), axis=0)
        
            xx_l = np.linspace(geo_old.x[il1], xl_[-1], nl_flap)
            yy_l = interp_from_curve(xx_l, xl_new2, yl_new2)
            xl_new = np.concatenate((geo_old.x[:il1], xx_l), axis=0)
            yl_new = np.concatenate((geo_old.yl[:il1], yy_l), axis=0)
            plt.plot(xu_new, yu_new)
            plt.plot(xl_new, yl_new)
            # =============================================================================
