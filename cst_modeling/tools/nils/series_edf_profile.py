...
class PoweredNacelleProfile():
    ...
    def calculate_segment_points(self) -> None:
        ...
        dx_te_rotor_to_le_stator = 5 * self.f_scale
        dx_te_stator_to_in_hx = 6 * self.f_scale
        
        dy_le_tip_inner_stator_to_le_inner_cowl = 0.5 * self.f_scale
        dy_le_tip_rotor_to_le_highlight = 0 * self.f_scale  # optionally replaced with something like theta_droop/theta_scarf
        ...
        x9 = x7
        x10 = x9 - self.params['l_hx_side']
        x11 = x10 - dx_te_stator_to_in_hx
        x12 = x11
        x13 = x12 + dx_te_stator_to_in_hx
        x14 = x13 + self.params['l_hx_side']
        x15 = x14 + self.params['l_core_nozzle']
        
        # Define body of revolution housing gear box, motor, and inverter
        self.zeta_max_core_body = self.params['r_hub_rotor'] / (x15 - x1)
        self.l_core_body = x15 - x1
        ...
        y10 = y9
        y11 = y5
        y12 = interpolate_core_body(xs,ys,x12,method='linear')
        y13 = interpolate_core_body(xs,ys,x13,method='linear')
        y14 = interpolate_core_body(xs,ys,x14,method='linear')
        y15 = 0.0
    ...
    def calculate_profile_segments(self) -> None:
        ...
        self.profile_segments[0] = self.core_body_profile(
            ...
            self.segment_points[14][0], self.segment_points[14][1]
        )
        #* (2,3)
        self.profile_segments[10] = self.core_body_profile(
            self.segment_points[1][0], self.segment_points[1][1],
            self.segment_points[2][0], self.segment_points[2][1]
        )
        #* (4,12)
        self.profile_segments[11] = self.core_body_profile(
            self.segment_points[3][0], self.segment_points[3][1],
            self.segment_points[11][0], self.segment_points[11][1]
        )
        #* (13,14)
        self.profile_segments[12] = self.core_body_profile(
            self.segment_points[12][0], self.segment_points[12][1],
            self.segment_points[13][0], self.segment_points[13][1]
        )
        
        #* (4,5) stator LE profile
        ...
        le_blade_data = np.array([
            [-0.73932092, 1.2191808866732865],
            [-0.78203724, 1.4405881811016372],
            [-0.805038335, 1.6636895926953357],
            [-0.808324206, 1.8651206354730736],
            [-0.78203724, 2.1088124801670576],
            [-0.752464403, 2.2678957119898455],
            [-0.706462212, 2.452534010297776],
            [-0.706462212, 2.6721601785220748],
        ])
        te_blade_data = np.array([
            [-0.377875137, 1.2813923493464086],
            [-0.338444688, 1.4530048827425248],
            [-0.318729463, 1.6603064765433868],
            [-0.312157722, 1.8789397180907148],
            [-0.315443593, 2.047660480494621],
            [-0.318729463, 2.244418626076097],
            [-0.338444688, 2.424967499564955],
            [-0.361445783, 2.58264835],
        ])
        
        self.le_blade_data_translated = le_blade_data.copy()
        self.le_blade_data_translated[:, 0] += self.segment_points[1][0] - self.le_blade_data_translated[0, 0]
        self.le_blade_data_translated[:, 1] += self.segment_points[1][1] - self.le_blade_data_translated[0, 1]
        
        self.te_blade_data_translated = te_blade_data.copy()
        self.te_blade_data_translated[:, 0] += self.segment_points[2][0] - self.te_blade_data_translated[0, 0]
        self.te_blade_data_translated[:, 1] += self.segment_points[2][1] - self.te_blade_data_translated[0, 1]
        
        self.le_blade_data_translated_scaled = self.le_blade_data_translated.copy()
        self.le_blade_data_translated_scaled[:, 1] = (
            self.le_blade_data_translated_scaled[0, 1] +
            (le_tip[1] - self.segment_points[1][1]) / (self.le_blade_data_translated_scaled[-1, 1] - self.le_blade_data_translated_scaled[0, 1]) *
            (self.le_blade_data_translated_scaled[:, 1] - self.le_blade_data_translated_scaled[0, 1])
        )
        self.le_blade_data_translated_scaled[:, 0] = (
            self.le_blade_data_translated_scaled[0, 0] +
            (self.segment_points[2][0] - self.segment_points[1][0]) / (te_blade_data[0, 0] - le_blade_data[0, 0]) *
            (self.le_blade_data_translated_scaled[:, 0] - self.le_blade_data_translated_scaled[0, 0])
        )
        
        self.te_blade_data_translated_scaled = self.te_blade_data_translated.copy()
        self.te_blade_data_translated_scaled[:, 1] = (
            self.te_blade_data_translated_scaled[0, 1] +
            (te_tip[1] - self.segment_points[2][1]) / (self.te_blade_data_translated_scaled[-1, 1] - self.te_blade_data_translated_scaled[0, 1]) *
            (self.te_blade_data_translated_scaled[:, 1] - self.te_blade_data_translated_scaled[0, 1])
        )
        self.te_blade_data_translated_scaled[:, 0] = (
            self.te_blade_data_translated_scaled[0, 0] +
            (self.segment_points[2][0] - self.segment_points[1][0]) / (te_blade_data[0, 0] - le_blade_data[0, 0]) *
            (self.te_blade_data_translated_scaled[:, 0] - self.te_blade_data_translated_scaled[0, 0])
        )
        
        x_le = self.le_blade_data_translated_scaled[:, 0]
        y_le = self.le_blade_data_translated_scaled[:, 1]
        self.ys_le = np.linspace(min(y_le), max(y_le), num=50)
        self.xs_makima_le = Akima1DInterpolator(y_le, x_le, method="makima")(self.ys_le)
        
        self.profile_segments[1][:, 0] = self.xs_makima_le
        self.profile_segments[1][:, 1] = self.ys_le
        
        x_te = self.te_blade_data_translated_scaled[:, 0]
        y_te = self.te_blade_data_translated_scaled[:, 1]
        self.ys_te = np.linspace(min(y_te), max(y_te), num=50)
        self.xs_makima_te = Akima1DInterpolator(y_te, x_te, method="makima")(self.ys_te)
        
        self.profile_segments[2][:, 0] = self.xs_makima_te
        self.profile_segments[2][:, 1] = self.ys_te
        # =============================================================================
        
        # =============================================================================
        self.profile_segments[13] = np.zeros((self.n_point_segment, 2))
        self.profile_segments[13][:, 0] = rotor_tip_profile[:, 0]
        self.profile_segments[13][:, 1] = rotor_tip_profile[:, 1]
        # =============================================================================
        ...
        # =============================================================================
        self.profile_segments[14] = np.zeros((self.n_point_segment, 2))
        self.profile_segments[14][:, 0] = np.linspace(self.segment_points[4][0], self.segment_points[10][0], self.n_point_segment)
        self.profile_segments[14][:, 1] = bypass_duct_inner_interp(self.profile_segments[14][:, 0])
        
        self.profile_segments[15] = np.zeros((self.n_point_segment, 2))
        self.profile_segments[15][:, 0] = np.linspace(self.segment_points[9][0], self.segment_points[8][0], self.n_point_segment)
        self.profile_segments[15][:, 1] = bypass_duct_inner_interp(self.profile_segments[15][:, 0])
        # =============================================================================
        ...
        #* (12,17) HX TE
        self.profile_segments[9] = self.create_straight_line(
            self.segment_points[8][0], self.segment_points[8][1],
            self.segment_points[13][0], self.segment_points[13][1]
        )
        
        #* (13,16)) HX LE
        self.profile_segments[8] = self.create_straight_line(
            self.segment_points[9][0], self.segment_points[9][1],
            self.segment_points[12][0], self.segment_points[12][1]
        )
        
        #* (14,15) outer stator TE profile
        te_tip = self.segment_points[10]
        te_hub = self.segment_points[11]
        te_tip[0] += 0.5 * self.f_scale
        te_hub[0] += 0.5 * self.f_scale
        avg_te = np.average((self.segment_points[11], te_tip), axis=0)
