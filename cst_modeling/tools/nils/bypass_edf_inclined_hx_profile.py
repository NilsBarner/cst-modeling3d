...
class PoweredNacelleProfile():
    ...
    def get_profile(self) -> Dict[np.ndarray, np.ndarray]:
        ...
        # self.profile_x = [value[:, 0] for key, value in self.profile_segments.items() if np.all(np.isfinite(value[:, 0]))]  # NILS: added finiteness check on 10.03.2026 to prevent issues when building 3D mesh
        # self.profile_y = [value[:, 1] for key, value in self.profile_segments.items() if np.all(np.isfinite(value[:, 1]))]  # NILS: added finiteness check on 10.03.2026 to prevent issues when building 3D mesh
    ...
    def calculate_segment_points(self) -> None:
        ...
        # self.f_scale = 0.06
        self.f_scale = 1
        dx_te_rotor_to_le_inner_cowl = 5 * self.f_scale
        dx_le_inner_cowl_to_le_outer_stator = 2 * self.f_scale
        ...
        x11 = x10 + self.params['l_bp_nozzle']
        x12 = x10
    ...
    def calculate_profile_segments(self) -> None:
        ...
        self.profile_segments[30][:, 0] = self.xs_makima_le
        self.profile_segments[30][:, 1] = self.ys_le
        ...
        self.profile_segments[31][:, 0] = self.xs_makima_te
        self.profile_segments[31][:, 1] = self.ys_te
        # =============================================================================
        ...
        self.profile_segments[10], func = self.bypass_duct_inner_profile(control_points, self.segment_points[10], self.segment_points[7], self.beta_outer)
        print('self.profile_segments[10] =', self.profile_segments[10])
        
        # =============================================================================
        angle_hinge = np.rad2deg(self.params['beta_bp_nozzle'])  # -10
        x_hinge = self.segment_points[10][0] - self.params['l_bp_nozzle']  #/ 2  # 40  # DIVISION BY TWO TEMPORARY
        ...
        # =============================================================================
        #* (12,11) HX upper side
        self.profile_segments[50] = self.create_straight_line(
            self.segment_points[12][0], self.segment_points[12][1],
            self.segment_points[11][0], self.segment_points[11][1]
        )
        self.profile_segments[50][:, 0] = np.linspace(self.segment_points[12][0], self.segment_points[11][0], 101)
        self.profile_segments[50][:, 1] = func(self.profile_segments[50][:, 0])
        # #* (12,11) HX lower side
        # self.profile_segments[51] = self.create_straight_line(
        #     self.segment_points[15][0], self.segment_points[15][1],
        #     self.segment_points[16][0], self.segment_points[16][1]
        # )
        # =============================================================================
        ...
        #* (19,20) use spline with tangency constraints but without control points
        control_points = np.array([])
        ...
        self.profile_segments[20], _ = self.bypass_duct_inner_profile(control_points, self.segment_points[18], self.segment_points[19], self.beta_inner)
        
        # =============================================================================
        angle_hinge = np.rad2deg(self.params['beta_core_nozzle'])  # -10
        x_hinge = self.segment_points[18][0] - self.params['l_core_nozzle']  # / 2  # 40  # DIVISION BY TWO TEMPORARY
        ...
        self.xu_old, self.xl_old, self.yu_old, self.yl_old = self.add_flap_to_airfoil(
            _foil_xu, _foil_xl, _foil_yu, _foil_yl, angle_hinge, x_hinge,
        )
        # =============================================================================
    ...
    def plot(self, show=True) -> None:
        ...
        # # =============================================================================
        # # ax.scatter(self.le_blade_data_translated_scaled[:, 0], self.le_blade_data_translated_scaled[:, 1])
        # # ax.scatter(self.te_blade_data_translated_scaled[:, 0], self.te_blade_data_translated_scaled[:, 1])
        # ax.scatter(self.xs_makima_le, self.ys_le)
        # ax.scatter(self.xs_makima_te, self.ys_te)
        # # =============================================================================
